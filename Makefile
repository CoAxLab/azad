...
# -------------------------------------------------------------------------
# 5-14-2020
# b024024747b6e586fa17bc3bc4bca4be023fedc7
# 
...
wythoff_exp68:
	...
	-mkdir $(DATA_PATH)/wythoff/exp68
	run_azad.py create_grid $(DATA_PATH)/wythoff/exp68/grid.csv --num_gpu=4 \
		--learning_rate='(0.0025, 0.25, 50)' \
		--epsilon='(0.1, 0.5, 5)' \
		--gamma='(0.1, 0.5, 5)' 
	parallel -j 32 -v \
		--joblog '$(DATA_PATH)/wythoff/exp68/exp68.parallel.log' \
		--nice 19 --delay 2 --header : --colsep ',' \
...
# TODO fix naming for each opp
# exp73
wythoff_eval73a:
	parallel -j 20 -v \
		--nice 19 \
		"run_azad.py evaluate_dqn2 $(DATA_PATH)/wythoff/exp73/run_DQN_optuna_{}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=self --debug=False --save=$(DATA_PATH)/wythoff/exp73/eval_DQN_optuna_self_{} --monitor='('episode', 'total_reward', 'winner', 'score')' --network=DQN_optuna --return_none=True"  ::: {2..22}

wythoff_eval73b:
	parallel -j 20 -v \
		--nice 19 \
		"run_azad.py evaluate_dqn2 $(DATA_PATH)/wythoff/exp73/run_DQN_optuna_{}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=random --debug=False --save=$(DATA_PATH)/wythoff/exp73/eval_DQN_optuna_random_{} --monitor='('episode', 'total_reward', 'winner', 'score')' --network=DQN_optuna --return_none=True" ::: {2..22}

wythoff_eval73c:
	parallel -j 20 -v \
		--nice 19 \
		"run_azad.py evaluate_dqn2 $(DATA_PATH)/wythoff/exp73/run_DQN_optuna_{}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=optimal --debug=False --save=$(DATA_PATH)/wythoff/exp73/eval_DQN_optuna_optimal_{} --monitor='('episode', 'total_reward', 'winner', 'score')' --network=DQN_optuna --return_none=True" ::: {2..22}
...
wythoff_eval73e:
	parallel -j 20 -v \
		--nice 19 \
		"run_azad.py evaluate_dqn2 $(DATA_PATH)/wythoff/exp73/run_{2}_{1}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=random --debug=False --save=$(DATA_PATH)/wythoff/exp73/eval_{2}_random_{1} --monitor='('episode', 'total_reward', 'score')' --network=DQN_optuna --return_none=True" ::: {2..22} ::: DQN_xy1 DQN_xy1 DQN_xy2 DQN_xy3 DQN_xy4 
		
wythoff_eval63a:
	parallel -j 20 -v \
		--nice 19 \
		"run_azad.py evaluate_dqn3 $(DATA_PATH)/wythoff/exp63/run_{2}_{1}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=random --debug=False --save=$(DATA_PATH)/wythoff/exp63/eval_{2}_random_{1} --monitor='('episode', 'total_reward', 'score')' --network=DQN_optuna --return_none=True" ::: {2..22} ::: DQN_hot1 DQN_hot1 DQN_hot2 DQN_hot3 DQN_hot4 DQN_hot5 DQN_conv2 DQN_conv3
