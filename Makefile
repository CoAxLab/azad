...
wythoff_eval73a:
	parallel -j 20 -v \
		--nice 19 --delay 2 \
		"run_azad.py evaluate_dqn2 $(DATA_PATH)/wythoff/exp73/run_{}_{}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=self --debug=False --save=$(DATA_PATH)/wythoff/exp73/eval_self_DQN_optuna_{} --monitor='('episode', 'total_reward', 'wins')' --network=DQN_optuna --return_none=True"  ::: {2..22}

wythoff_eval73b:
	parallel -j 20 -v \
		--nice 19 --delay 2 \
		"run_azad.py evaluate_dqn2 $(DATA_PATH)/wythoff/exp73/run_{}_{}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=random --debug=False --save=$(DATA_PATH)/wythoff/exp73/eval_random_DQN_optuna_{} --monitor='('episode', 'total_reward', 'wins')' --network=DQN_optuna --return_none=True" ::: {2..22}

wythoff_eval73c:
	parallel -j 20 -v \
		--nice 19 --delay 2 \
		"run_azad.py evaluate_dqn2 $(DATA_PATH)/wythoff/exp73/run_{}_{}.pytorch --game=Wythoff15x15 --num_episodes=1e2 --opponent=optimal --debug=False --save=$(DATA_PATH)/wythoff/exp73/eval_optimal_DQN_optuna_{} --monitor='('episode', 'total_reward', 'wins')' --network=DQN_optuna --return_none=True" ::: {2..22}
