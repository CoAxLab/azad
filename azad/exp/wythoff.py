...
class Game(object):
    ...
    def create_player(self, name, **kwargs):
        ...
        Player = getattr(thismodule, name)
    ...
    def play(self, N=10, game='Wythoff3x3', path=None):
        ...
        for trial in range(N):
            ...
            state = (x, y, board, moves)
            done = False

            # ----------------------------------------------------------------
            while not done:
                # Player plays
                state, done, self.env = self.player.step(state, done, self.env)
        ...
        if path is not None:
            ...
            self.opponent.save(os.path.join(path, "opponent"))
        ...
        return self.player, self.opponent, self.env


class WythoffQTable(object):
    def __init__(self,
                 learning_rate=0.1,
                 epsilon=0.1,
                 gamma=0.98,
                 default_Q=0.0):
        """A lookup agent for playing Wythoff's game"""
    ...
    def backward(self, board, move, Q, max_Q, reward):
        """Q learning update"""
    ...
    def step(self, state, done, env, learn=True):
        ...
        # Freeze a copt for grad updates at the end
        self.grad_board = deepcopy(board)

        # --------------------------------------------------------------------
        # Get values and make decisions:
        # Q(s, A)
        try:
            Qs = self.model[board]
        except KeyError:
        ...
        # Decide
        move_i = _np_epsilon_greedy(Qs, epsilon=self.epsilon)
        move = moves[move_i]

        # Freeze for grad updates at the end
        self.grad_move_index = deepcopy(move_i)
        self.grad_move = deepcopy(move)
        ...
        # --------------------------------------------------------------------
        # Play
        (x, y, board, moves), self.reward, done, _ = env.step(move)
        try:
            board = tuple(flatten_board(board).numpy())
        except AttributeError:
            pass
