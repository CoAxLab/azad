...
def shift(turn, player, opponent):
    if turn == 0:
        return 1, player
    elif turn == 1:
        return 0, opponent
...
def wythoff_dqn4(epsilon=0.1,
    ...
    player_target = Model().to(device)
    opponent = Model().to(device)
    opponent_target = Model().to(device)

    if double:
        player_target.load_state_dict(player.state_dict())
        player_target.eval()
        opponent_target.load_state_dict(opponent.state_dict())
    ...
    for episode in range(1, num_episodes + 1):
        ...
        # -------------------------------------------------------------------
        # Init turn taking
        turn, current = 0, player

        # Play a game
        steps = 1
        done = False
        while not done:
            # Take turns {0: player, 1: opponent}
            turn, current = shift(turn, player, opponent)
        ...
        for i in range(0, len(transitions), 1):
            player_memory.push(*transitions[i])
        for i in range(0, len(transitions), 2):
            opponent_memory.push(*transitions[i])
        ...
        #     writer.add_scalar('player_reward', reward, episode)
    ...
    if player_target is not None:
        result['player_target'] = player_target.state_dict()
    if opponent_target is not None:
        result['opponent_target'] = opponent_target.state_dict()

    # Save the result, return, all that jazz.
    if save:
